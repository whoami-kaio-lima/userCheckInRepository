...
router.get("/:fullname/justify/denied/:id",adminAuth.authenticateLogin, adminAuth.authenticateLowUser, async (req, res) => {
    const {id} = req.params;
    const hour = await Hours.findByPk(id)
    res.render("users/justifyDenied.ejs", {justifydenied:hour.justifydenied});
});


router.get("/:fullname/justify/:id",adminAuth.authenticateLogin, adminAuth.authenticateLowUser,  async (req, res) => {
    const { id } = req.params;
    try {
...
router.post("/save/justify", async (req, res) => { // Corrigido o caminho da rota para corresponder ao formulário
    try {
...
router.post("/:fullname/filter", adminAuth.authenticateLogin, async (req, res) => {
    const { date } = req.body;
    const userId = req.session.user.id;

    if (!date) {
        return res.status(400).render('users/filtro.ejs', { errorMessage: "Por favor, selecione uma data para filtrar.", filteredHours: [], user: req.session.user });
    }
    ...
    try {
        ...
        res.render('users/filtro.ejs', { filteredHours: formattedHours, user: req.session.user, errorMessage: formattedHours.length === 0 ? "Nenhum dado encontrado." : null });
    } catch (error) {
...
router.post("/save/time", adminAuth.authenticateLogin, async (req, res) => {
    let { entradaTime, saidaTime } = req.body;

    const userId = req.session.user.id;

    if (entradaTime == null && saidaTime == null) {
        console.log("Redirecionando para /user/logged devido a entradas nulas.");
        res.redirect("/user/logged");
    } else {
        const formatoEntrada = `${entradaTime}_${userId}`;
        const formatoSaida = `${saidaTime}_${userId}`;
        console.log("Formato de entrada:", formatoEntrada);
        console.log("Formato de saída:", formatoSaida);
        try {
            await Hours.create({
            ...
            console.log("Horários salvos com sucesso, redirecionando para /user/logout.");
        } catch (error) {
...
router.get("/:name", adminAuth.authenticateLogin, adminAuth.authenticateLowUser,  async (req, res) => {
    ...
    if (user) {
        ...
        const formattedHours = hours.map(hour => {
            const formattedDate = new Date(hour.createdAt).toLocaleDateString('pt-BR');
            return {
                ...hour.dataValues,
...
router.post("/login/authenticate", async (req, res) => {
    ...
    else if (cpfOrEmailOrLogin.match(/^\d{3}\.\d{3}\.\d{3}-\d{2}$/) || cpfOrEmailOrLogin.match(/^\d{11}$/)) {
    ...
    // Caso não se encaixe em nenhum padrão reconhecido
        ...
        res.render('users/login', { error: "Formato inválido para CPF, email ou nome." });
        return;
    }
    ...
    if (user && user.isActive) {
        ...
        if (comparison) {
            ...
            route ? res.redirect(route) : res.status(403).send("Função de usuário inválida");
        } else {
