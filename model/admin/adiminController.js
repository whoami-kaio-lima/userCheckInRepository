...
router.get("/request", adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    try {
...
router.get("/deny/:id",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    const { id } = req.params;
...
router.post("/deny/adjustment", async (req, res) => {
    ...
    const negacao = await Hours.update({ request: false, denied: true, justifydenied:justify},{where: {id:id}})
    res.redirect("/admin/schedules")
})


router.get("/adjustment/:id",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    const { id } = req.params;
...
router.get("/justify/:id",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    const { id } = req.params;
    try {
        const hour = await Hours.findByPk(id);
        if (hour) {
...
router.get("/schedules",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    try {
...
router.post("/schedules/filter", async (req, res) => {
    ...
    try {
        ...
        if (user) {
            res.render("admin/schedulesFiltered.ejs", { users, selectedUser: user, hours: user.hours || [] });
        } else {
...
router.get("/horario/:id",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
...
router.post('/save/new', upload.single('image'), async (req, res) => {
    let { name, surname, cpf, email, password, roles, institution, course, period, shift, prohibited, exit } = req.body;
...
router.get("/create/new",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    res.render("admin/new.ejs")
});

router.get("/delete/:id",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
...
router.get("/deactivate/:id",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    const userId = req.params.id;
...
router.get("/toggle/:id",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    const userId = req.params.id;
    ...
    try {
        if (userId !== req.session.user.id) {
            ...
            await Users.update({ isActive: newStatus }, {
                where: {
                    id: userId
                }
            });
...
router.post("/update", async (req, res) => {
    let { id, name, surname, cpf, email, password, roles, institution, course, period, shift, prohibited, exit } = req.body;
    name = name.toUpperCase();
    surname = surname.toUpperCase();
...
router.get("/main",adminAuth.authenticateLogin, adminAuth.authenticateADM, async (req, res) => {
    try {
        const users = await Users.findAll();
